...
`docs/ROADMAP.md` carries open work only, and is the file to update when something lands. `docs/TUTORIAL.md` is the one narrative path for a new practitioner and owns the widget foreground example; `docs/HOWTO.md` is the reference and links to it rather than repeating it. `docs/TROUBLESHOOTING.md` is generated from the pipeline's actual `raise` sites — when you add or reword a guard, update it there too. The
