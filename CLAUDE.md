...
- **Brightway project name:** `asphalt-lca` (kept from the origin project so existing brightway data
  and validated results carry over unchanged; the name is historical, not scope-defining).
...
Two directories: `setup/` (shared, one-time per machine) and `general/` (practitioner-facing —
foreground CSV or USLCI UUID → LCIA results, no openLCA dependency).

Run once per machine, in order: `setup/00` → `setup/01` → `setup/02` → `setup/03b` → `setup/03`.
Then run `general/04` per study. `setup/03b` must run before `setup/03` — it injects the electricity
