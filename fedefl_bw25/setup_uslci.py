...
`import_uslci()` is the whole script's body as a function: same code, same order,
same numbers — the only changes are that what used to come from the environment
(`USLCI_FULL_DB`, `SOURCE_DATA_DIR`, `ALLOW_UNIT_PASSTHROUGH`) now arrives as
arguments, that progress goes to an optional `log` callable instead of `print`,
and that overwriting an existing database takes `overwrite=`/`confirm=` instead
...
def import_uslci(*, full_db=False, bundle_dir=None, allow_unit_passthrough=False,
    ...
    _CONV_TABLE_PATH = Path(CONV_TABLE_PATH)
    ...
    if FULL_DB_MODE:
        ...
        zip_files = [_full]
        log(f"FULL-DB MODE: importing the entire USLCI database from {_full.name}")
        log(f"  target database: '{USLCI_DB_NAME}' "
    ...
    log(f"  Source identity: {_source_identity['mode']}, "
    ...
    if EXTERNAL_PROVIDER_DB in bd.databases:
        ...
        log(f"Loaded {len(external_provider_uuids)} external provider process(es) "
    ...
    if USLCI_DB_NAME in bd.databases:
        ...
        if not (overwrite or (confirm(USLCI_DB_NAME) if confirm else False)):
            raise UslciExists(
                f"Database '{USLCI_DB_NAME}' already exists. Pass overwrite=True "
    ...
    log(f"Database '{USLCI_DB_NAME}' written — {len(db_data)} processes"
          + (f" [electricity_vintage = {electricity_vintage}]." if electricity_vintage else "."))
    log(f"  Biosphere exchanges: {total_bio_matched} matched, {total_bio_unmatched} unmatched")
    log(f"  Technosphere exchanges: {total_tech_linked} linked "
    ...
    log(f"  Provenance sidecar: {_DB_PROVENANCE_PATH} ({len(proc_provenance)} processes)")
